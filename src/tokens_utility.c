    ...
    int *inside_quotes)
{
    ...
    char *token, buf[4096];
    int backslash = 0;
    TRACEE("Passed str [%s] inside_quotes [%d]\n", str, *inside_quotes);
    ...
    *read_bytes = 0ul;
    ...
    buf[i] = '\0';
    token = strdup(buf);
    TRACEL("Extracted token [%s] last_ch [%c] read_bytes [%lu] " \
...
{
    int exp_sp = 0;
    char *token_str, last_ch = '\0';
    size_t read_bytes = 0ul;
    st_token *head = NULL, *tail = NULL, *item;
    ...
    while (str)
    {
        ...
        token_str = read_token(str, &last_ch, &read_bytes, &exp_sp);
        if (read_bytes == 0ul)
            break;
        str += read_bytes;

        item = st_token_create(token_str);
        st_token_push_back(&tail, item);
        assert(tail);
        if (tail->next) /* on first iteration next will be null */
        {
            TRACE("Token '%s' '%p' has been added to token '%s' '%p'\n",
                item->str, (void *) item, tail->str, (void *) tail);
            tail = tail->next;
        }
        else
        {
            TRACE("First token has been added to the list\n");
            head = tail;
        }
        FREE(token_str);
